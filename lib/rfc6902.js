...
var _, _simpleDiff, _cleanup, diff;
_ = require('lodash');
_simpleDiff = function(lhs, rhs, path){
  ...
  if (isCollection.l && isCollection.r) {
    ...
    for (i$ = 0, len$ = keys.length; i$ < len$; ++i$) {
      key = keys[i$];
      result = result.concat(_simpleDiff(lhs[key], rhs[key], path + "/" + key));
    }
  } else {
        ...
        if (patch !== added) {
          if (patch.op === 'remove') {
            if (patch.path === added.path) {
              ...
              break;
            } else if (patch.value === added.value) {
              result.push({
                op: 'move',
                from: patch.path,
                path: added.path
          ...
          } else if (patch.op === 'nop' && patch.value === added.value) {
            result.push({
              op: 'copy',
              from: patch.path,
              path: added.path
...
module.exports = {
  simpleDiff: function(lhs, rhs){
    return _cleanup(_simpleDiff(lhs, rhs));
  },
  diff: diff
};